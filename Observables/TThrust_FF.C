...
namespace RESUM {
  ...
  class TThrust_FF: public Observable_Base {
  public:

    TThrust_FF(const Observable_Key &args): 
    ...
    Obs_Params Parameters
      (const std::vector<Vec4D>& p,
       ...
       const size_t &l) {
      double d=log(1.0/(2.0*p[2].PPerp2()/(p[0]*p[1])));
      d+=-2.0*log(2.0);
      return Obs_Params(1.0,1.0,d,0.0);
    }
  ...
  };// end of class TThrust_FF

}// end of namespace RESUM

using namespace RESUM;

DECLARE_GETTER(TThrust_FF,"TThrust_FF",Observable_Base,Observable_Key);
Observable_Base *ATOOLS::Getter<Observable_Base,Observable_Key,TThrust_FF>::
operator()(const Parameter_Type &args) const 
{ return new TThrust_FF(args); }
void ATOOLS::Getter<Observable_Base,Observable_Key,TThrust_FF>::
